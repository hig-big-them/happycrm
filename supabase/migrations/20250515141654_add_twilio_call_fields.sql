...
-- deadline_datetime sütunun indexini de oluştur (performans için)
DO $$ 
BEGIN
  -- Eğer index yoksa oluştur
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'transfers' AND indexname = 'transfers_deadline_datetime_idx'
  ) THEN
    CREATE INDEX transfers_deadline_datetime_idx ON public.transfers (deadline_datetime);
  END IF;
END $$;
